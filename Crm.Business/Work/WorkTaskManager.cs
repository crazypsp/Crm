...
namespace Crm.Business.Work
{
    public sealed class WorkTaskManager : IWorkTaskManager
    {
        ...
        public WorkTaskManager(CrmDbContext db) => _db = db;

        public async Task<WorkTask> CreateTaskAsync(Guid tenantId, Guid? companyId, string title, string? description, DateTimeOffset? dueAt, int priority, CancellationToken ct)
        {
        ...
        public async Task AssignUserAsync(Guid tenantId, Guid taskId, Guid userId, bool isOwner, CancellationToken ct)
        {
            Guard.NotEmpty(tenantId, nameof(tenantId));
            Guard.NotEmpty(taskId, nameof(taskId));
            Guard.NotEmpty(userId, nameof(userId));

            var task = await _db.WorkTasks.FirstOrDefaultAsync(x => x.Id == taskId && x.TenantId == tenantId && !x.IsDeleted, ct)
                       ?? throw new NotFoundException("Görev bulunamadı.");

            var user = await _db.Users.FirstOrDefaultAsync(x => x.Id == userId, ct)
                       ?? throw new NotFoundException("Kullanıcı bulunamadı.");

            // Basit tenant kontrolü: user.TenantId eşleşmeli (MVP). İleri: Membership üzerinden kontrol edilir.
            if (user.TenantId != tenantId)
                throw new ForbiddenException("Kullanıcı bu tenant’a ait değil.");

            var exists = await _db.WorkTaskAssignments.AnyAsync(x => x.WorkTaskId == taskId && x.UserId == userId && x.TenantId == tenantId, ct);
            if (!exists)
            {
        ...
        public async Task SetStatusAsync(Guid tenantId, Guid taskId, int status, CancellationToken ct)
        {
            Guard.NotEmpty(tenantId, nameof(tenantId));
            Guard.NotEmpty(taskId, nameof(taskId));
