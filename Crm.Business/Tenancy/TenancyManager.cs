...
namespace Crm.Business.Tenancy
{
    public sealed class TenancyManager : ITenancyManager
    {
        ...
        public TenancyManager(CrmDbContext db) => _db = db;
        ...
        public async Task<Tenant> CreateTenantAsync(Guid dealerId, string officeName, CancellationToken ct)
        {
            ...
            var dealerExists = await _db.Dealers.AnyAsync(x => x.Id == dealerId && !x.IsDeleted, ct);
            if (!dealerExists) throw new NotFoundException("Bayi bulunamadı.");
        ...
        public async Task<Company> CreateCompanyAsync(Guid tenantId, string title, string? taxNo, CancellationToken ct)
        {
            ...
            if (tenant is null) throw new NotFoundException("Tenant (mali müşavir ofisi) bulunamadı.");
            if (!tenant.IsActive) throw new ValidationException("Tenant pasif durumda.");
        ...
        public async Task<Tenant> GetTenantAsync(Guid tenantId, CancellationToken ct)
        {
            ...
            return t ?? throw new NotFoundException("Tenant bulunamadı.");
        }
        ...
        public async Task<Company> GetCompanyAsync(Guid tenantId, Guid companyId, CancellationToken ct)
        {
            ...
            return c ?? throw new NotFoundException("Firma bulunamadı veya yetkisiz erişim.");
        }
    }
}
